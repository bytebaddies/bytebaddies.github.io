...
export default function Home() {
  ...
  const updateStateAndNavigate = (data: FetchData): void => {
    ...
    router.push(
      `/tada?${createQueryString('imageUrl', data.image)}&${createQueryString('name', data.name)}&${createQueryString('artist', data.artist)}&${createQueryString('songUrl', data.url)}`,
    )
  }
    ...
    <main className="relative flex flex-col bg-[#2b2b2b] h-screen w-screen place-items-center justify-center">
      <div className=" flex justify-center place-items-center grid grid-cols-1 grid-rows-3 w-fit h-fit">
        ...
        <div className="flex justify-center justify-items-center grid grid-cols-2 mt-[1rem]">
          <button
            className="flex justify-center h-fit font-archivo font-normal text-[#2B2B2B] bg-[#D7C2A1] w-fit py-2 px-16 rounded-md"
            onClick={() => {
              handleSearch()
            }}
          >
          ...
          <button
            className="flex justify-center h-fit font-archivo font-normal text-[#E4E2DE] bg-[#A12D1E] w-fit py-2 px-10 rounded-md"
            onClick={() => {
              handleLucky()
            }}
          >
